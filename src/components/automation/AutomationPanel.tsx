...
export function AutomationPanel() {
  const { activeRun, panelOpen, setPanelOpen } = useAutomation();
  if (!panelOpen) return null;

  return (
    <>
      <div className="fixed inset-0 z-40 bg-black/50 backdrop-blur-sm" onClick={() => setPanelOpen(false)} aria-hidden />
      <aside className="fixed right-0 top-0 z-50 flex h-full w-full max-w-md flex-col border-l border-[var(--nova-border)] bg-[var(--nova-bg-secondary)] shadow-2xl">
